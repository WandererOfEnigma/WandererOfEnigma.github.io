document.addEventListener('DOMContentLoaded', function() {
    ...
    if (client) {
        client.onMessageArrived = function(message) {
            ...
            // Check the topic to determine the type of message
            if (topic === 'ENGO551/Arnold/my_temperature') {
                // This is a temperature update message
                ...
                // This is a location update message
                let coordinates = payload.geometry.coordinates;

                // Update map with your current location
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            let message = JSON.stringify(geoJson);
            let topic = '<your_topic_for_location>'; // Replace with your topic for location
            let messageObject = new Paho.MQTT.Message(message);
            messageObject.destinationName = topic;
            client.send(messageObject);
        }, function(error) {
